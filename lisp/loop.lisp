...
(defun loop-optimization-quantities (env) ;!!!
  ...
  (declare #-(or ANSI eclipse) (ignore env)
...
(defun make-standard-loop-universe (&key keywords for-keywords iteration-keywords path-keywords
  ...
  #+(and anal-retentive (not (and CLOE Source-Bootstrap)))
...
;;; If not NIL, this must be a list of two things: a LET-like macro,
;;; and a SETQ-like macro, which perform LOOP-style destructuring. 
...
(defun loop-constant-fold-if-possible (form &optional expected-type)
  ...
  (let ((new-form form) (constantp nil) (constant-value nil))
    ...
    #-Genera (when (setq constantp (constantp new-form))
	       (setq constant-value (cl:eval new-form)))
...
;;; Causes the iteration to terminate "normally", the same as
;;; implicit termination by an iteration driving clause, or by use of
;;; WHILE or UNTIL -- the epilogue code (if any) will be run, and any
...
(defun add-loop-path (names function universe &key preposition-groups inclusive-permitted user-data)
  ...
  #+(and anal-retentive (not (or Genera (and CLOE Source-Bootstrap))))
...
(defun loop-hash-table-iteration-path (variable data-type prep-phrases &key which)
  #+anal-retentive (check-type which (member hash-key hash-value))
...
(defun loop-standard-expansion (keywords-and-forms environment universe) 
  (let ((sample (car keywords-and-forms)))
    (if (and sample (symbolp sample))
	(loop-translate keywords-and-forms environment universe)
	(let ((tag (gensym "LOOP")))
	  `(block nil (tagbody ,tag (progn ,@keywords-and-forms)
			       (go ,tag)))))))

(defvar *loop-ansi-universe* (make-ansi-loop-universe nil))
